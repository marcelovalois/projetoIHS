...
static void* led_verde = NULL;
static void* led_vermelho = NULL;
...
static long int my_ioctl(struct file*, unsigned int cmd, unsigned long arg)
{
	...
	case WR_R_DISPLAY:
		...
		break;
	case WR_GREEN_LEDS:
		write_pointer = led_verde;
		write_name_index = 2 + 2;
		break;
	case WR_RED_LEDS:
		write_pointer = led_vermelho;
...
static int my_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	led_verde = ioremap(resource + 0xC0C0, 0x20);
	led_vermelho = ioremap(resource + 0xc100, 0x20);
